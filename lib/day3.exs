...
defmodule CalculatePowerConsumption do
  ...
  def get_oxigen_generator_rating(diagnostic_codes, position) do
    if length(diagnostic_codes) <= 1 do
      ...
      remaining_diagnostic_codes = Enum.filter(diagnostic_codes, fn code -> String.at(code, position) == "#{max_digit}" end)
  ...
  def get_co2_scrubber_rating(diagnostic_codes, position) do
    if length(diagnostic_codes) <= 1 do
      ...
      remaining_diagnostic_codes = Enum.filter(diagnostic_codes, fn code -> String.at(code, position) == "#{min_digit}" end)
  ...
  def calculate_life_support_raiting(diagnostic_codes) do
    ...
    IO.puts "oxigen generator rating: #{oxigen_generator_rating}, co2 scrubber rating: #{co2_scrubber_rating}, life support rating: #{oxigen_generator_rating * co2_scrubber_rating}"

  end

  def digit_to_binary(digits) do
    Enum.reduce(Enum.with_index(Enum.reverse(digits)), 0, fn {digit, index}, decimal_number -> decimal_number + (:math.pow(2, index) |> round) * digit   end)
  end
  ...
  def calculate_power_consumption(diagnostic_codes) do
    ...
    gamma_rate = digit_to_binary(max_digits) # Enum.reduce(Enum.with_index(Enum.reverse(max_digits)), 0, fn {digit, index}, decimal_number -> decimal_number + (:math.pow(2, index) |> round) * digit   end)
    epsilon_rate = Enum.reduce(Enum.with_index(Enum.reverse(max_digits)), 0, fn {digit, index}, decimal_number -> decimal_number + (:math.pow(2, index) |> round) * (1 - digit)   end)

    IO.puts "gamma rate #{gamma_rate}, epsilon rate #{epsilon_rate}, power consumption #{gamma_rate * epsilon_rate}"
  end
